...
test('owner TOTP provisioning remains unconfirmed until valid code confirmation', () => {
  const store=new InMemoryOwnerTotpStore(), id='owner-1', p=provisionOwnerTotp(store,id);
  assert.ok(p.secret); assert.match(p.otpauthUri,/^otpauth:\/\/totp\/MarketNiora/); assert.equal(store.get(id).confirmedAt,null);
  assert.equal(confirmOwnerTotp(store,id,'000000'),false); assert.equal(confirmOwnerTotp(store,id,generateTotpCode(p.secret)),true); assert.ok(store.get(id).confirmedAt);
});
test('owner MFA challenge is single-use and expires', () => {
  ...
  totp.set(id,{secret,confirmedAt:now}); const challenge=beginOwnerMfa(totp,ch,id,now); assert.ok(challenge);
  assert.equal(verifyOwnerMfa(totp,ch,challenge,'000000',now).success,false); assert.equal(verifyOwnerMfa(totp,ch,challenge,generateTotpCode(secret,now),now).success,true); assert.equal(verifyOwnerMfa(totp,ch,challenge,generateTotpCode(secret,now),now).success,false);
  const expired=beginOwnerMfa(totp,ch,id,now); assert.equal(verifyOwnerMfa(totp,ch,expired,generateTotpCode(secret,now),now+300000).success,false);
});
